...
import { LayoutDashboard, PieChart, Target, Sparkles, Plus } from 'lucide-react';
...
const NavItem = ({ id, icon: Icon, label, active, onClick }: any) => (
  ...
  >
    ...
    <span className="text-[7px] font-[900] uppercase tracking-[0.2em] whitespace-nowrap">{label}</span>
  </button>
);
...
export const Layout: React.FC<LayoutProps> = ({ children, activeTab, onTabChange, onAddTransaction }) => {
  return (
    <div className="flex flex-col w-full h-[100dvh] bg-background text-foreground overflow-hidden">
      ...
      {/* Modern Navigation Bar - Optimized for Mobile Viewports */}
      <nav className="h-[85px] w-full shrink-0 relative z-[100] bg-transparent pb-safe-bottom">
        <div className="max-w-md mx-auto h-full relative">
          
          {/* Visual Background Layer - Gradient Shadow instead of Black shadow */}
          <div className="absolute inset-x-0 bottom-0 h-[100px] pointer-events-none">
            <svg width="100%" height="100%" viewBox="0 0 400 100" preserveAspectRatio="none" className="drop-shadow-[0_-15px_35px_rgba(139,92,246,0.12)]">
              <path 
